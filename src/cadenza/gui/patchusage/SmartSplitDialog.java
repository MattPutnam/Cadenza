...
public class SmartSplitDialog extends OKCancelDialog {
  ...
  private SmartSplitPane _smartSplitPane;
  ...
  protected JComponent buildContent() {
    _smartSplitPane = new SmartSplitPane();
    
    _patchUsageCombo = new JComboBox<>(_others.toArray(new PatchUsage[_others.size()]));
    _patchUsageCombo.addActionListener(e -> _smartSplitPane.notifyChange());
    if (_patchUsage.isSplit())
      _patchUsageCombo.setSelectedItem(_patchUsage.splitTwin);
    
    final Box top = SwingUtils.buildCenteredRow(new JLabel("Merge with:"), _patchUsageCombo);
    
    final JPanel panel = new JPanel(new BorderLayout());
    panel.add(SwingUtils.buildCenteredRow(top), BorderLayout.NORTH);
    panel.add(_smartSplitPane, BorderLayout.CENTER);
    return panel;
  }
  ...
  protected void takeActionOnOK() {
    _smartSplitPane.apply();
  }
  
  private class SmartSplitPane extends JPanel {
